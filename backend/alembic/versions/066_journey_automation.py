...
def upgrade():
    ...
    op.create_index("ix_journeys_active_trigger", "journeys", ["is_active", "trigger_type"])
    ...
    op.create_index("ix_journey_steps_journey_order", "journey_steps", ["journey_id", "order"])
    ...
    op.create_table(
        ...
        sa.Column("current_step_id", sa.String(36), sa.ForeignKey("journey_steps.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_index("ix_user_journeys_user_journey", "user_journeys", ["user_id", "journey_id"], unique=True)
    op.create_index("ix_user_journeys_started_at", "user_journeys", ["started_at"])
    op.create_index("ix_user_journeys_status_started", "user_journeys", ["status", "started_at"])

    # step_executions table
    op.create_table(
        "step_executions",
        sa.Column("id", sa.String(36), primary_key=True),
        sa.Column("user_journey_id", sa.String(36), sa.ForeignKey("user_journeys.id", ondelete="CASCADE"), nullable=False),
        sa.Column("step_id", sa.String(36), sa.ForeignKey("journey_steps.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("ix_step_executions_user_journey_id", "step_executions", ["user_journey_id"])
    op.create_index("ix_step_executions_step_id", "step_executions", ["step_id"])
    op.create_index("ix_step_executions_executed_at", "step_executions", ["executed_at"])
    op.create_index("ix_step_executions_status_executed", "step_executions", ["status", "executed_at"])
    op.create_index("ix_step_executions_user_journey_step", "step_executions", ["user_journey_id", "step_id"])
