...
# Set encryption key for tests - generate a valid Fernet key
...
def generate_device_auth_headers(
    device: Device, method: str, path: str, body_dict: dict
) -> dict:
    """
    ...
    Args:
        ...
        path: Request path (e.g., /api/v1/client/ack)
        body_dict: Request body as dict (will be JSON serialized)
...
async def test_ack_successful_placement_creates_open_position(
    ...
    # Flush to write to database but keep transaction open
    # This allows both test and endpoint to see the same data within the transaction
...
async def test_ack_failed_execution_does_not_create_position(
    ...
    # Use flush instead of commit to persist data while keeping transaction open
    await db_session.flush()
