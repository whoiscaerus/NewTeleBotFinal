...
# === Telegram Bot Info ===
TELEGRAM_BOT_TOKEN: str = "8195080561:AAGX79Zt3OTt8X5AnNTovlm2mYN7F3izPLA"
...
LIVE_POSITION_UPDATE_INTERVAL: float = 5  
...
def is_trading_restricted(now: datetime) -> bool:
    """
    Check if trading is restricted due to weekly open, daily open, or daily gap.

    Args:
        now: Current datetime in broker's server timezone.

    Returns:
        bool: True if trading is restricted, False otherwise.
    """
    try:
        # Convert server time to UK time (Europe/London)
        uk_tz = pytz.timezone('Europe/London')
        server_tz = pytz.timezone('UTC')  # Adjust to your broker's server timezone if different
        now_server = now.replace(tzinfo=server_tz)
        now_uk = now_server.astimezone(uk_tz)

        # Weekly open: Sunday 23:00–23:30 UK time
        if now_uk.weekday() == 6:  # Sunday
            weekly_open_start = now_uk.replace(hour=23, minute=0, second=0, microsecond=0)
            weekly_open_end = weekly_open_start + timedelta(minutes=30)
            if weekly_open_start <= now_uk < weekly_open_end:
                logging.info("Trading restricted: Within 30 minutes of weekly open (Sunday 23:00–23:30 UK).")
                return True

        # Daily open: Monday–Friday 23:00–23:30 UK time
        if now_uk.weekday() in [0, 1, 2, 3, 4]:  # Monday to Friday
            daily_open_start = now_uk.replace(hour=23, minute=0, second=0, microsecond=0)
            daily_open_end = daily_open_start + timedelta(minutes=30)
            if daily_open_start <= now_uk < daily_open_end:
                logging.info("Trading restricted: Within 30 minutes of daily open (23:00–23:30 UK).")
                return True

        # Daily gap: Monday–Friday 22:00–23:00 UK time
        if now_uk.weekday() in [0, 1, 2, 3, 4]:  # Monday to Friday
            daily_gap_start = now_uk.replace(hour=22, minute=0, second=0, microsecond=0)
            daily_gap_end = daily_gap_start + timedelta(hours=1)
            if daily_gap_start <= now_uk < daily_gap_end:
                logging.info("Trading restricted: Within daily gap period (22:00–23:00 UK).")
                return True

            # Prevent new trades 15 minutes before daily gap (21:45–22:00 UK)
            pre_gap_start = daily_gap_start - timedelta(minutes=15)
            if pre_gap_start <= now_uk < daily_gap_start:
                logging.info("Trading restricted: Within 15 minutes before daily gap (21:45–22:00 UK).")
                return True

        return False

    except Exception as e:
        logging.error(f"Error checking trading restrictions: {e}")
        return True  # Default to restricted if there's an error
...
class TelegramRateLimiter:
    """Token bucket rate limiter for Telegram API calls per chat."""
    def __init__(self, rate: float = 15, per: float = 60):
        """
        Initialize rate limiter.
        Args:
            rate: Number of allowed requests (tokens) per period.
            per: Time period in seconds (e.g., 60 for one minute).
        """
        self.rate = rate
        self.per = per
        self.buckets: Dict[str, tuple[float, float]] = {}  # chat_id -> (tokens, last_refill)
        self.lock = Lock()

    def _refill(self, chat_id: str) -> None:
        """Refill tokens for the given chat_id based on elapsed time."""
        now = time.time()
        with self.lock:
            if chat_id not in self.buckets:
                self.buckets[chat_id] = (self.rate, now)
            tokens, last_refill = self.buckets[chat_id]
            elapsed = now - last_refill
            new_tokens = tokens + (elapsed / self.per) * self.rate
            self.buckets[chat_id] = (min(new_tokens, self.rate), now)

    def can_send(self, chat_id: str) -> bool:
        """Check if a message can be sent for the given chat_id."""
        self._refill(chat_id)
        with self.lock:
            tokens, last_refill = self.buckets[chat_id]
            if tokens >= 1:
                self.buckets[chat_id] = (tokens - 1, last_refill)
                return True
            return False

# Initialize rate limiter globally
telegram_rate_limiter = TelegramRateLimiter(rate=15, per=60)  # 15 requests per minute

def rate_limited_notify(
    ...
    """
    Wrapper for notify() that respects rate limits.
    Returns True if message was sent, False if rate limited.
    """
    if telegram_rate_limiter.can_send(chat_id):
        notify(text, chat_id, reply_markup, parse_mode)
        return True
    else:
        logging.warning(f"Rate limit exceeded for chat_id {chat_id}. Skipping notification.")
        return False
...
def update_live_position_message(chat_id: str = TELEGRAM_USER_ID) -> None:
    """
    Send or update a Telegram message with live position details every 5 seconds.
    ...
    try:
        ...
        if not positions:
            if live_position_message_id:
                ...
                if rate_limited_notify(text, chat_id, parse_mode="MarkdownV2"):
                    try:
...
def execute_trade(
    ...
    """
    Execute a trade based on the model's action prediction.

    Args:
        action: Integer action from PPO model (0: HOLD, 1: BUY, 2: CLOSE_BUY).
        action_label: String label of the action ("HOLD", "BUY", "CLOSE_BUY").
        current_price: Current market price for the trade.
        features: Dictionary of feature values used for the prediction.
    """
    global mt5_failure_count, user_position_size
    logging.debug(f"Executing trade: Action={action_label}, Price={current_price:.2f}")
    print(f"[{datetime.now()}] Decision: {action_label} at ${current_price:.2f}")
    ...
    try:
        ...
        if not symbol_info or not account_info:
            mt5_failure_count += 1
            logging.error("Failed to fetch symbol or account info.")
            rate_limited_notify(
                "❌ **Error:** Unable to fetch market data for trade execution.",
                parse_mode="MarkdownV2"
            )
            return

        equity_gbp: float = account_info.equity * EXCHANGE_RATE
        logging.debug(f"Raw equity: {account_info.equity}, Converted equity: £{equity_gbp:.2f}")

        if equity_gbp < MIN_EQUITY_THRESHOLD:
            logging.warning(f"Equity £{equity_gbp:.2f} below minimum £{MIN_EQUITY_THRESHOLD}. Trading halted.")
            rate_limited_notify(
                f"⚠️ **Low Equity Alert:** £{equity_gbp:.2f} < £{MIN_EQUITY_THRESHOLD}. Trading halted.",
                parse_mode="MarkdownV2"
            )
            return
        ...
        if action == 1:  # BUY
            if positions:
                logging.info("Position already open. Skipping BUY.")
                if notification_settings.get("HOLD", True):
                    rate_limited_notify(
                        f"🛑 **HOLD {SYMBOL}**: Position already open at ${current_price:.2f}.",
                        parse_mode="MarkdownV2"
                    )
                return

            if position_size < MIN_POSITION_SIZE:
                logging.error(f"Position size {position_size} below minimum {MIN_POSITION_SIZE}.")
                rate_limited_notify(
                    f"❌ **Invalid Position Size:** {position_size} < {MIN_POSITION_SIZE}.",
                    parse_mode="MarkdownV2"
                )
                return
            ...
            if result.retcode == mt5.TRADE_RETCODE_DONE:
                logging.info(f"BUY executed: {SYMBOL}, Size={position_size}, Price=${current_price:.2f}")
                ...
                log_trade(
                    datetime.now(), "BUY", SYMBOL, current_price, position_size,
                    unit_label, 0.0, equity_gbp, "PPO BUY", features
                )
                mt5_failure_count = 0
            else:
                mt5_failure_count += 1
                logging.error(f"BUY failed: {result.comment}")
                rate_limited_notify(
                    f"❌ **BUY Failed:** {result.comment}",
                    parse_mode="MarkdownV2"
                )

        elif action == 2:  # CLOSE_BUY
            if not positions:
                logging.info("No open position to close. Skipping CLOSE_BUY.")
                if notification_settings.get("HOLD", True):
                    rate_limited_notify(
                        f"🛑 **HOLD {SYMBOL}**: No open position at ${current_price:.2f}.",
                        parse_mode="MarkdownV2"
                    )
                return
            ...
            if result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                pips: float = round((current_price - open_price) * 10, 1)
                logging.info(f"CLOSE_BUY executed: {SYMBOL}, P/L=£{profit_loss_gbp:.2f}, Pips={pips:.1f}")
                if notification_settings.get("CLOSE_BUY", True):
                    rate_limited_notify(
                        f"✅ **CLOSE BUY {SYMBOL}**\n"
                        f"💵 **Price:** ${current_price:.2f}\n"
                        f"📏 **Size:** {position_volume} {unit_label}\n"
                ...
                log_trade(
                    ...
                    unit_label, profit_loss_gbp, equity_gbp, "PPO CLOSE", features
                )
                mt5_failure_count = 0
            else:
                mt5_failure_count += 1
                logging.error(f"CLOSE_BUY failed: {result.comment}")
                rate_limited_notify(
                    f"❌ **CLOSE BUY Failed:** {result.comment}",
                    parse_mode="MarkdownV2"
                )

        else:  # HOLD (action == 0)
            logging.info(f"HOLD: {SYMBOL} at ${current_price:.2f}")
            if notification_settings.get("HOLD", True):
                rate_limited_notify(
...
def main() -> None:
    ...
    if not mt5.initialize(login=MT5_LOGIN, server=MT5_SERVER, password=MT5_PASSWORD, path=MT5_PATH):
        ...
        logging.error(error_msg)
        rate_limited_notify(error_msg, parse_mode="MarkdownV2")
        sys.exit(1)
    logging.info("MT5 initialized successfully.")
    rate_limited_notify("✅ **MT5 Initialized Successfully!**", parse_mode="MarkdownV2")

    # Initialize trade database
    init_trade_db()
    rate_limited_notify("📊 **Trade Database Initialized!**", parse_mode="MarkdownV2")
    ...
    try:
        ...
        rate_limited_notify("🤖 **Model and Scaler Loaded Successfully!**", parse_mode="MarkdownV2")
    except Exception as e:
    ...
    rate_limited_notify("📲 **Telegram Updates Thread Started!**", parse_mode="MarkdownV2")

    # Send startup message and simulate /start command
    rate_limited_notify("🚀 **Trading Bot Started!**", parse_mode="MarkdownV2")
    ...
    try:
        while not stop_flag:
            ...
            if mt5_failure_count >= MT5_MAX_FAILURES:
                rate_limited_notify(
                    f"⚠️ **MT5 Circuit Breaker Tripped:** {mt5_failure_count} failures. Pausing for {MT5_BACKOFF_TIME} seconds.",
                    parse_mode="MarkdownV2"
                )
            ...
            if (now - last_state_log).total_seconds() >= STATE_LOG_INTERVAL:
                ...
                telegram_rate_limiter.log_usage()  # Log Telegram API usage
                last_state_log = now
                ...
                if feature_sequence is not None and features is not None:
                    ...
                    if tick:
                        ...
                        if notification_settings.get(action_label, True):
                            rate_limited_notify(
                                f"📈 **Action:** {action_label} | **Price:** ${current_price:.2f}",
                                parse_mode="MarkdownV2"
                            )
                    else:
                        mt5_failure_count += 1
                        ...
                        rate_limited_notify(
                            "❌ **Error:** Failed to fetch current price for trade execution.",
                            parse_mode="MarkdownV2"
                        )
                else:
                    mt5_failure_count += 1
                    ...
                    rate_limited_notify(
                        "❌ **Error:** Failed to fetch market data for prediction.",
                        parse_mode="MarkdownV2"
                    )
            ...
            if scheduled_reports and (now - last_report_time).total_seconds() >= REPORT_INTERVAL:
                ...
                with open(ANALYTICS_REPORT_PATH, "w") as f:
                    f.write(report_text)
                rate_limited_notify(report_text, parse_mode="MarkdownV2")
            ...
            if position_monitoring_enabled and (now - last_position_update).total_seconds() >= 60:
                ...
                if positions:
                    ...
                    rate_limited_notify(position_text, parse_mode="MarkdownV2")
            ...
            if heartbeat_interval and (now - last_heartbeat_time).total_seconds() >= heartbeat_interval:
                with heartbeat_lock:
                    rate_limited_notify("💓 **I'm Alive!** 🫡", parse_mode="MarkdownV2")
                ...
                while not command_queue.empty():
                    ...
                    if action == "dashboard":
                        try:
                            ...
                            dashboard_text, equity_chart, drawdown_chart = generate_dashboard(start_date, end_date)
                            rate_limited_notify(dashboard_text, chat_id, parse_mode="MarkdownV2")
                            if equity_chart:
        ...
        rate_limited_notify("🛑 **Bot Stopped by User.**", parse_mode="MarkdownV2")
    except Exception as e:
        ...
        rate_limited_notify("🔚 **Bot Shutdown Complete.**", parse_mode="MarkdownV2")
        sys.exit(0)