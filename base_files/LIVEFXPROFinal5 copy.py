...
MAX_DRAWDOWN_THRESHOLD: float = 1  # 10% max drawdown
...
notification_settings: Dict[str, bool] = {
    ...
    "DRAWDOWN": True  # Control drawdown alert notifications
}
...
def is_trading_allowed() -> Tuple[bool, bool, str]:
    """
    Check if trading is allowed based on broker's trading session times, market open/close, and weekend constraints.

    Returns:
        Tuple of (allow_new_trades, allow_holding_positions, status_message):
        - allow_new_trades: True if new trades can be opened.
        - allow_holding_positions: True if existing positions can be held.
        - status_message: Reason why trading is restricted, if applicable.
    """
    try:
        with mt5_lock:
        ...
        if not symbol_info:
            mt5_failure_count += 1
            logging.error("Failed to fetch symbol info for trading session check.")
            return False, False, "Failed to fetch symbol info."
        ...
        if current_day >= 5:
            return False, False, "Trading disabled during weekend (Saturday/Sunday)."

        # Get trading session times (assuming daily sessions are available)
        # Note: session_times is not directly accessible in MT5 Python API; we use trading hours from symbol_info
        # For simplicity, assume GOLD trades 24/5 with specific daily open/close times
        # We will check if the market is open and apply the 30-min post-open and 15-min pre-close rules

        # Typical GOLD trading hours (UTC): Sunday 23:00 to Friday 22:00
        # We approximate daily sessions; actual session times may vary by broker
        market_open_time = now.replace(hour=23, minute=0, second=0, microsecond=0)
        if current_day == 0:  # Adjust for Monday (after Sunday open)
            market_open_time = now.replace(hour=23, minute=0, second=0, microsecond=0) - timedelta(days=1)
        market_close_time = now.replace(hour=22, minute=0, second=0, microsecond=0)
        if current_day == 4:  # Friday close
            market_close_time = now.replace(hour=22, minute=0, second=0, microsecond=0)

        # Check if market is open
        if not symbol_info.trade_mode == mt5.SYMBOL_TRADE_MODE_FULL:
            return False, False, "Market is closed (trade mode restricted)."

        # Check 30 minutes after market open (no new trades)
        open_window_end = market_open_time + timedelta(minutes=30)
        if market_open_time <= now < open_window_end and current_day != 5:
            return False, True, "No new trades within 30 minutes of market open."

        # Check 15 minutes before market close (no holding positions)
        close_window_start = market_close_time - timedelta(minutes=15)
        if close_window_start <= now <= market_close_time:
            return False, False, "No trading or holding 15 minutes before market close."

        # Check Friday close to prevent holding over weekend
        if current_day == 4 and now >= (market_close_time - timedelta(minutes=15)):
            return False, False, "No trading or holding before Friday market close to avoid weekend positions."

        return True, True, "Trading allowed."
    except Exception as e:
        mt5_failure_count += 1
        logging.error(f"Error checking trading session: {e}")
        return False, False, f"Error checking trading session: {e}"
...
def check_alerts() -> None:
    ...
    try:
        ...
        if current_drawdown > 0.05 and not drawdown_alert_triggered and notification_settings["DRAWDOWN"]:
    ...
    while not stop_flag:
        try:
            ...
            for update in updates:
                ...
                if text.startswith("/start"):
                    ...
                    if len(parts) == 2:
                        ...
                        if notify_type in notification_settings:
                            ...
                            notify(
                                "❌ *Invalid Notification Type:* Use BUY, CLOSE_BUY, HOLD, or DRAWDOWN.",
                                chat_id,
                                parse_mode="MarkdownV2"
                            )
                    else:
                ...
                elif text.startswith("/resetdrawdown"):
                    global drawdown_alert_triggered
                    drawdown_alert_triggered = False
                    notify(
                        "🔄 *Drawdown Alert Reset:* Drawdown alerts will trigger again if threshold is exceeded.",
                        chat_id,
                        parse_mode="MarkdownV2"
                    )
                    ...
                    if notify_type in notification_settings:
                        ...
                        # Refresh the inline keyboard to reflect updated notification status
                        send_inline_keyboard(chat_id)
                    else:
                        notify(
                ...
                elif callback_data == "reset_drawdown":
                    drawdown_alert_triggered = False
                    notify(
                        "🔄 *Drawdown Alert Reset:* Drawdown alerts will trigger again if threshold is exceeded.",
                        chat_id,
...
def main() -> None:
    ...
    Closes trades when holding is not allowed (e.g., 15 min pre-close), pauses new trades for 30 min post-open,
    and resumes trading when conditions allow, while keeping the bot running.
    """
    ...
    global live_position_tracking, last_live_position_update, live_position_logging
    ...
    last_mt5_check: datetime = datetime.now()
    STATE_LOG_INTERVAL: int = 300
    MT5_CHECK_INTERVAL: int = 60
    ...
    try:
        while not stop_flag:
            ...
            if (now - last_mt5_check).total_seconds() >= MT5_CHECK_INTERVAL:
                if not mt5.terminal_info():
                    mt5_failure_count += 1
                    error_msg = f"⚠️ **MT5 Disconnected:** {mt5.last_error()}"
                    logging.error(error_msg)
                    notify(error_msg)
            ...
            allow_new_trades, allow_holding_positions, trading_status = is_trading_allowed()
            logging.debug(f"Trading session check: allow_new_trades={allow_new_trades}, "
                          f"allow_holding_positions={allow_holding_positions}, status={trading_status}")

            # Close all open positions if holding is not allowed
            if not allow_holding_positions:
                with mt5_lock:
                    # Verify MT5 connection before proceeding
                    if not mt5.terminal_info():
                        mt5_failure_count += 1
                        error_msg = f"❌ **MT5 Disconnected Before Auto-Close:** {mt5.last_error()}"
                        logging.error(error_msg)
                        notify(error_msg)
                        continue
                    positions = mt5.positions_get(symbol=SYMBOL)
                    tick = mt5.symbol_info_tick(SYMBOL)
                    account_info = mt5.account_info()
                if positions and tick and account_info:
                    for position in positions:
                        ...
                        request: Dict[str, Any] = {
                            ...
                            "deviation": 20,
                            "magic": 234000,
                            "comment": f"Auto CLOSE: {trading_status}",
                            "type_time": mt5.ORDER_TIME_GTC,
                            "type_filling": mt5.ORDER_FILLING_IOC,
                        }
                        result = mt5.order_send(request)
                        if result is None:
                            mt5_failure_count += 1
                            error_msg = f"❗ **Auto CLOSE Failed:** mt5.order_send returned None. Last error: {mt5.last_error()}"
                            logging.error(error_msg)
                            notify(error_msg)
                        elif result.retcode == mt5.TRADE_RETCODE_DONE:
                            profit_loss_gbp: float = get_trade_profit_gbp(position_ticket)
                            notify(
                                f"✅ **Auto CLOSE BUY {SYMBOL}**\n"
                                f"---\n"
                                f"💵 **Price:** $ {current_price:.2f}\n"
                                f"📏 **Size:** {position_volume} lots\n"
                                f"💷 **P/L:** £{profit_loss_gbp:.2f} | Pips: {pips:+.1f}\n"
                                f"💷 **Equity:** £{equity_gbp:.2f}\n"
                                f"📝 **Reason:** {trading_status}",
                                parse_mode="MarkdownV2"
                            )
                            log_trade(
                                datetime.now(), "CLOSE_BUY", SYMBOL, current_price, position_volume,
                                "lots", profit_loss_gbp, equity_gbp, f"Auto CLOSE: {trading_status}", {}
                            )
                        else:
                            mt5_failure_count += 1
                            error_msg = f"❗ **Auto CLOSE Failed:** {result.comment} (Retcode: {result.retcode})"
                            logging.error(error_msg)
                            notify(error_msg)
                elif not tick or not account_info:
                    mt5_failure_count += 1
                    logging.error("Failed to fetch market data for auto-close.")
                    notify("❌ **Error:** Failed to fetch market data for auto-close.")

            # Update trading_paused based on trading session constraints
            if not allow_new_trades:
                if not trading_paused:
                    trading_paused = True
                    notify(f"⏸ **Trading Paused:** {trading_status}")
                    logging.info(f"Trading paused: {trading_status}")
            else:
                if trading_paused and not stop_flag:
                    trading_paused = False
                    notify("▶️ **Trading Resumed:** Trading allowed.")
                    logging.info("Trading resumed: Trading allowed.")

            # Check for new candle and execute trading logic
            if not trading_paused and allow_new_trades and is_new_candle():
                ...
                logging.info(f"Trading skipped: {trading_status}")
                notify(f"ℹ️ **Trading Skipped:** {trading_status}")
